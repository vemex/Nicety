<template>
    <table
            cellspacing="0"
            cellpadding="0"
            class="nicety-date-table"
            @click="handleClick"
            @mousemove="handleMouseMove"
            :class="{ 'is-week-mode': selectionMode === 'week' }">
        <tbody>
        <tr>
            <th v-if="showWeekNumber">{{ t('nicety.datepicker.week') }}</th>
            <th v-for="(week, key) in WEEKS" :key="key">{{ t('nicety.datepicker.weeks.' + week) }}</th>
        </tr>
        <tr
                class="nicety-date-table__row"
