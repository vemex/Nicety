...
const getI18nSettings = () => {
    return {
        dayNamesShort: weeks.map(week => t(`nicety.datepicker.weeks.${week}`)),
        dayNames: weeks.map(week => t(`nicety.datepicker.weeks.${week}`)),
        monthNamesShort: months.map(month => t(`nicety.datepicker.months.${month}`)),
        monthNames: months.map((month, index) => t(`nicety.datepicker.month${index + 1}`)),
        amPm: ['am', 'pm']
    };
};
