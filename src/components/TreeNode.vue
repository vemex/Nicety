<template>
    <ul class="nicety tree">
        <li v-for="(item, index) in data" :key="item.key"
            ...
            <i v-else>&nbsp;</i>
            <nicety-checkbox :index="item.key"
                           :state="dataKeyMap[item.key][0]"
                           @click.stop="onItemChecked(item)"
                           ref="checkboxes" v-if="multiple"></nicety-checkbox>
...
<script>
    import NicetyCheckbox from './Checkbox.vue'
    export default {
        components: {NicetyCheckbox},
        name: 'nicety-tree-node',
        props: {
