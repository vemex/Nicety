    ...
    <div class="data-grid" :style="gridStyleObj">
        <div ref="toolPanel" class="data-grid-tool-panel"
            ...
            <template v-for="item in operations">
                <button type="button" class="btn btn-outline-primary btn-sm" @click="handleAction(item.type)" ><i :class="item.icon"></i>{{item.display}}</button>
            </template>
        </div>
        <ag-grid-vue style="width: 100%; height: 100%" class="ag-theme-balham" :columnDefs="columnDefObj" :rowData="rowData"  :gridReady="onGridReady"
